  ...
  async function buscar() {
    try {
      ...
      if (sku.trim()) {
        ...
        url = `${BASE_URL}/productos/by-codigo/${encodeURIComponent(code.trim())}`;
      } else {
...
function Ingreso() {
  ...
  async function guardar() {
    try {
      ...
      const r = await fetch(`${BASE_URL}/ingresos`, {
        ...
        body: JSON.stringify({ items: [{ productoId: pid, cantidad: qty, costoUnitario: price }] }),
      });
      if (!r.ok) throw new Error((await r.text()) || `HTTP ${r.status}`);

      Alert.alert("OK", "Ingreso registrado");
      setSku(""); setProductoId(""); setCantidad(""); setCosto("");
    } catch (e: any) {
  ...
  return (
    <ScrollView contentContainerStyle={s.wrap}>
      ...
      <TouchableOpacity style={[s.btn, loading && { opacity: 0.7 }]} onPress={guardar} disabled={loading}>
