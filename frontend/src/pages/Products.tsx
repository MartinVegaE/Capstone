...
export default function ProductsPage() {
  ...
  const [sortBy, setSortBy] = useState<"sku" | "nombre" | "marca" | "categoria" | "stock">("nombre");
  const [sortDir, setSortDir] = useState<SortDir>("asc");

  // Data
  // PARCHE: tipamos el objeto como 'any' para no chocar con la firma actual del hook
const { data, isLoading, isError, refetch } = useProducts({
  ...
  const brands = useMemo(
    () => Array.from(new Set(rows.map((p) => p.marca).filter(Boolean))).sort(),
    [rows]
  );
  const categories = useMemo(
    () => Array.from(new Set(rows.map((p) => p.categoria).filter(Boolean))).sort(),
    [rows]
  );
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const dto = {
      ...
      nombre: String(fd.get("nombre") || editing.nombre || ""),
  ...
  const renderCard = (p: Product) => (
    <div>
      <div className="mb-1 flex items-center justify-between">
        ...
        <span className="rounded bg-slate-100 px-2 py-0.5 text-xs font-mono">{p.sku}</span>
      </div>
      <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
        {(p as any).marca && <span className="rounded bg-slate-100 px-2 py-0.5">{(p as any).marca}</span>}
        {(p as any).categoria && <span className="rounded bg-slate-100 px-2 py-0.5">{(p as any).categoria}</span>}
        <span className="rounded bg-emerald-50 px-2 py-0.5 text-emerald-700">Stock: {fmtInt(p.stock)}</span>
      </div>
    </div>
  );
  ...
  return (
    <section className="w-full px-6 pb-6">
      ...
      <div className="w-full px-6 pt-3">
        <div className="flex flex-wrap items-center gap-2 text-sm">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {(brand || category) && (
              <Button variant="secondary" onClick={() => { setBrand(""); setCategory(""); setPage(1); }}>
                Limpiar filtros
              </Button>
            )}
          </div>
        ...
        <>
          ...
          {isError && (
            <div className="w-full px-6 py-6 text-red-700">
              Error al cargar productos. <Button size="sm" onClick={() => refetch()}>Reintentar</Button>
            </div>
          )}
          {!isLoading && !isError && (
            <KanbanBoard<Product>
              ...
              <table className="w-full text-[13px]">
                <thead className="sticky top-0 z-10 bg-slate-50 text-slate-600 shadow-[0_1px_0_0_rgba(0,0,0,0.05)]">
                  <tr>
                    ...
                    <Th onClick={() => toggleSort("nombre")} active={sortBy === "nombre"} dir={sortDir}>Nombre</Th>
                    <Th onClick={() => toggleSort("marca")} active={sortBy === "marca"} dir={sortDir}>Marca</Th>
                    <Th onClick={() => toggleSort("categoria")} active={sortBy === "categoria"} dir={sortDir}>Categoría</Th>
                    <Th onClick={() => toggleSort("stock")} active={sortBy === "stock"} dir={sortDir}>Stock</Th>
                ...
                <tbody>
                  ...
                  {!isLoading && !isError && rows.map((p, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5">{(p as any).nombre ?? (p as any).name ?? "-"}</td>
                      <td className="px-4 py-2.5">{(p as any).marca ?? ""}</td>
                      <td className="px-4 py-2.5">{(p as any).categoria ?? ""}</td>
      ...
      {/* Drawer editar */}
      <Drawer open={editing !== null} onClose={closeDrawer} title={editing ? `Editar ${editing.sku}` : "Producto"}>
        {editing && (
          <form onSubmit={onSubmit} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  name="nombre"
                  defaultValue={(editing as any).nombre ?? (editing as any).name ?? ""}
